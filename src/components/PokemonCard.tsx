...
import { Pokemon, TYPE_COLORS, PokemonType, getTypeEffectiveness } from '@/lib/pokemon-data';
...
// Get type effectiveness info for tooltips
function getTypeTooltipInfo(attackerType: PokemonType): string {
	...
	for (const defType of allTypes) {
		const eff = getTypeEffectiveness(attackerType, [defType]);
		if (eff >= 2) strong.push(defType);
		else if (eff <= 0.5 && eff > 0) weak.push(defType);
		else if (eff === 0) noEffect.push(defType);
	}
	
	let info = `${attackerType} is:`;
	if (strong.length > 0) info += `\n🟢 Strong vs: ${strong.join(', ')}`;
	if (weak.length > 0) info += `\n🟡 Weak vs: ${weak.join(', ')}`;
	if (noEffect.length > 0) info += `\n⚪ No effect vs: ${noEffect.join(', ')}`;
...
export default function PokemonCard({
	pokemon, player, selectedMove, onSelectMove, isActive, isShaking, damageText, effectText, attacking = false, showEntrance = false
}: PokemonCardProps) {
	...
	// Prevent details from opening when clicking moves
	const handleCardClick = (e: React.MouseEvent) => {
		// Don't open details when clicking on move buttons
	...
	return (
		<>
			<div 
				className={`panel-gradient rounded-xl p-6 relative transition-all duration-300 card-hover-lift card-glow-hover ${getEntranceClass()} ${getShakeClass()} ${isShaking && !attacking ? 'animate-flash-damage' : ''}`}
						...
						<TooltipProvider key={t} delayDuration={200}>
							<Tooltip>
								<TooltipTrigger asChild>
									<span className={`${getTypeColorClass(t)} text-xs px-3 py-1 rounded-full font-semibold text-primary-foreground shadow-sm cursor-help`}>
										{t}
									</span>
								</TooltipTrigger>
								<TooltipContent className="max-w-xs text-xs bg-card border-border p-3 rounded-lg shadow-xl">
									...
									<div className="space-y-1">
										{strong.length > 0 && (
											<p className="text-green-400">🟢 <span className="text-foreground">Super effective:</span> {strong.join(', ')}</p>
										)}
										{weak.length > 0 && (
											<p className="text-amber-400">🟡 <span className="text-foreground">Not very effective:</span> {weak.join(', ')}</p>
										)}
										{noEffect.length > 0 && (
											<p className="text-slate-400">⚪ <span className="text-foreground">No effect:</span> {noEffect.join(', ')}</p>
										)}
									</div>
								</TooltipContent>
							</Tooltip>
...
<DialogContent className="bg-card border-border max-w-md mx-auto p-6 rounded-xl shadow-2xl">
					<DialogHeader>
						<DialogTitle className="font-pixel text-sm text-primary flex items-center gap-2">
							{pokemon.imageSrc ? (
								<img src={pokemon.imageSrc} alt={pokemon.name} className="w-8 h-8 object-contain" />
							) : (
